<?php 
 $this->load->view('navbar');
?>
        <div class="row no-gutters slider-text js-fullheight align-items-center justify-content-center" data-scrollax-parent="true">
          <div class="col-md-9 text-center ftco-animate" data-scrollax=" properties: { translateY: '70%' }">
            <p class="breadcrumbs" data-scrollax="properties: { translateY: '30%', opacity: 1.6 }"><span class="mr-2"><a href="index.html">Home</a></span> <span>CCTV</span></p>
            <h1 class="mb-3 bread" data-scrollax="properties: { translateY: '30%', opacity: 1.6 }">CCTV Lalu lintas Indonesia</h1>
          </div>
        </div>
      </div>
    </div>
    <section class="ftco-section bg-light">
      <div class="container">
        <div class="row d-flex">
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_1.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV Gede Bage kab bandung</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_2.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota Bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV TOL Buah Batu</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_3.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV TOL Buah Batu</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_4.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV Gede Bage kab bandung</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_5.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota Bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV TOL Buah Batu</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
          <div class="col-md-4 d-flex ftco-animate">
            <div class="blog-entry align-self-stretch">
              <a href="blog-single.html" class="block-20" style="background-image: url(<?php echo base_url().'assets/images/image_6.jpg'?>);">
              </a>
              <div class="text">
              	<span class="tag">CCTV kota bandung</span>
                <h3 class="heading mt-3"><a href="#">CCTV TOL Buah Batu</a></h3>
                <div class="meta mb-3">
                  <div><a href="#">November 3, 2018</a></div>
                  <div><a href="#">cctv</a></div>
                  <div><a href="#" class="meta-chat"><span class="icon-chat"></span> 3</a></div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="row mt-5">
          <div class="col text-center">
            <div class="block-27">
              <ul>
                <li><a href="#">&lt;</a></li>
                <li class="active"><span>1</span></li>
                <li><a href="#">2</a></li>
                <li><a href="#">3</a></li>
                <li><a href="#">4</a></li>
                <li><a href="#">5</a></li>
                <li><a href="#">&gt;</a></li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </section>
    <?php 
 $this->load->view('footer');
?>